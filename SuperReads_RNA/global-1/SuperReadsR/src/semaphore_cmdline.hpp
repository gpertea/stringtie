...
public:
  ...
  bool                           wait_flag;
  bool                           print_flag;
  ::std::vector<const char *>    cmd_arg;
  typedef ::std::vector<const char *>::iterator cmd_arg_it;
  typedef ::std::vector<const char *>::const_iterator cmd_arg_const_it;
  ...
  args_t() :
    id_arg(""), id_given(false),
    file_arg(""), file_given(false),
    jobs_arg((uint32_t)1), jobs_given(false),
    command_arg(""), command_given(false),
    wait_flag(false),
    print_flag(false),
    cmd_arg()
  { }
  ...
  void parse(int argc, char* argv[]) {
    ...
    static const char *short_options = "+hVUf:j:c:wp";
...
#define CHECK_ERR(type,val,which) if(!err.empty()) { ::std::cerr << "Invalid " #type " '" << val << "' for [" which "]: " << err << "\n"; exit(1); }
    while(true) {
      ...
      case 'j':
        jobs_given = true;
        jobs_arg = conv_uint<uint32_t>((const char*)optarg, err, false);
        CHECK_ERR(uint32_t, optarg, "-j, --jobs=uint32")
        break;
      case 'c':
        command_given = true;
        command_arg = optarg;
    ...
    if(id_given && file_given)
      error("Switches [    --id=string] and [-f, --file=string] are mutually exclusive");
    if(wait_flag && command_given)
      error("Switches [-w, --wait] and [-c, --command=string] are mutually exclusive");
    if(print_flag && wait_flag)
      error("Switches [-p, --print] and [-w, --wait] are mutually exclusive");
    if(print_flag && command_given)
      error("Switches [-p, --print] and [-c, --command=string] are mutually exclusive");

    // Parse arguments
    if(argc - optind < 0)
      error("Requires at least 0 argument.");
    for( ; optind < argc; ++optind) {
      cmd_arg.push_back(argv[optind]);
    }
  }
  static const char * usage() { return "Usage: semaphore [options] cmd:string+"; }
  class error {
    int code_;
  ...
  static const char * help() { return
    ...
    "     --id=string                          Name of semaphore\n"
    " -f, --file=string                        Path to semaphore file\n"
    " -j, --jobs=uint32                        Max number of jobs (1)\n"
    " -c, --command=string                     Command to run through the shell\n"
    " -w, --wait                               Wait for all command to be done (false)\n"
    " -p, --print                              Print the number of slots available (false)\n"
    " -U, --usage                              Usage\n"
    " -h, --help                               This message\n"
    " -V, --version                            Version";
  }
...
#define PACKAGE_VERSION "0.0.0"
  ...
  void dump(::std::ostream &os = std::cout) {
    os << "id_given:" << id_given << " id_arg:" << id_arg << "\n";
    os << "file_given:" << file_given << " file_arg:" << file_arg << "\n";
    os << "jobs_given:" << jobs_given << " jobs_arg:" << jobs_arg << "\n";
    os << "command_given:" << command_given << " command_arg:" << command_arg << "\n";
    os << "wait_flag:" << wait_flag << "\n";
    os << "print_flag:" << print_flag << "\n";
    os << "cmd_arg:" << vec_str(cmd_arg) << "\n";
  }
};
