...
bool good_junc(CJunction& jd,int refstart, GVec<float>* bpcov) {
	...
	if(jd.usg_start>=0) return true; // this passed the USG filtering
...
int build_usg(BundleData* bdata) {
	...
	//fprintf(stderr,"build_usg with %d guides\n",guides.Count());
	GVec<int> guidepred;     // keeps the prediction number associated with a guide --> here I might need to modify in conjunction with the USG
	GArray<GEdge> guideedge; // 0: negative starts; 1 positive starts
							 // guideedge's are smallest first/last exons of guides that start at a give position; i.e. if 100-200 and 100-300 are the first exons in two different guides, it will remember 100-200
	if(guides.Count()) {
		...
		//if(eonly)
		for(int g=0;g<guides.Count();g++) {
			guidepred.cAdd(-1); // none of the guides are associated with a prediction initially
			bool covered=true;
			RC_TData* tdata=(RC_TData*)(guides[g]->uptr);
			if(longreads || mixedMode) {  // because I corrected some of the junctions in this case, I need to do more work
			...
			else { // check if guide is fully covered by reads --> I might modify this in conjunction with the USG
			...
			if(covered) { // all junctions in guide g are covered
				tdata->in_bundle=2;
				...
				if(idx<0) guideedge.Add(ge); // if this edge is not present in guideedge than add it
				else if(guideedge[idx].endval>guides[g]->exons[0]->end) guideedge[idx].endval=guides[g]->exons[0]->end;
				if(s<0) { // if strand unknown, search for guide edge on the other strand as well
					ge.strand=1;
	...
	GList<CJunction> ejunction(junction); // sorted junctions according to their ends
	ejunction.setFreeItem(false);
	if(ejunction.Count()) ejunction.setSorted(juncCmpEnd);

	uint start=0;
	uint end=0;
	double leftsupport[2]={0,0};    // support is strand based
	double rightsupport[2]={0,0};
	bool higherr=false;
	char leftcons=-1;
	char rightcons=-1;
	for(int i=0;i<junction.Count();i++) { // I might rely on USG here instead of checking for all of these -> maybe only for longreads and mixedmode??
		...
		if((!higherr || mixedMode) && junction[i]->strand && junction[i]->nm==junction[i]->nreads && !junction[i]->guide_match) { // if higherr wasn't set up, do it now for the erroneous junctions; go through loop for mixedMode anyway to adjust junctions
			higherr=true;
			if(mixedMode) {
				int j=i-1;
				while(j>=0 && junction[j]->start+sserror>junction[i]->start) { // delete this higherr junction's strand if there is a better nearby junction
				...
				if(junction[i]->strand) { // if junction survided junctions coming before check for the following junctions
					j=i+1;
...
*/
	...
	bool resort=false; // tells if I have to resort junctions
	int njunc=junction.Count();
	for (int n=0;n<readlist.Count();n++) { // for all reads
		...
		while(i<rd.juncs.Count()) { // check all junctions in read
...
void count_good_junctions(BundleData* bdata) {
	...
	if((longreads||mixedMode) && bdata->keepguides.Count()) { // there are guides to consider with longreads -> I might need to adjust the splice sites
		...
		for(int g=0;g<guides.Count();g++) { // check all junction in guides and add them to gjunc
			char s=0; // unknown strand
			if(guides[g]->strand=='+') s=1; // guide on positive strand
			else if(guides[g]->strand=='-') s=-1; // guide on negative strand
			for(int i=1;i<guides[g]->exons.Count();i++) { //add_junction((int)guides[g]->exons[i-1]->end,(int)guides[g]->exons[i]->start,gjunc,s);
				...
				if(gidx>-1) { // item is new
					...
					if(mixedMode) { // this should favor junctions already covered by short reads; if junction is not covered by short read ignore it
						CJunction jn(guides[g]->exons[i-1]->end,guides[g]->exons[i]->start,s);
						int oidx=-1;
		...
		if(gjunc.Count()) { // if I found junctions in gjunc
			...
			GList<CJunction> ejunction(junction); // sorts all junctions based on ends (seems highly inefficient, given that I do this later too -> I might reconsider this for USG since my bundles could be huge
			ejunction.setFreeItem(false);
			if(ejunction.Count()) ejunction.setSorted(juncCmpEnd);
			GList<CJunction> egjunc(gjunc); // sort the gjunc as well
	...
	for(int n=0;n<readlist.Count();n++) {
		...
		else if(rdcount>1) rdcount=1; // this is for unitig (super-reads) only

		GVec<uint> leftsup;     // list of all left supports of junctions []---
		GVec<uint> rightsup;	// list of all right supports of junctions ---[]
		uint maxleftsupport=0;  // maximum left support of all junctions in read []---
		uint maxrightsupport=0; // maximum right support of all junctions in read ---[]

		//int sno=(int)rd.strand+1; // 0(-),1(.),2(+)
		//if(nex>1) fprintf(stderr,"Process spliced read[%d] with cov=%f and sno=%d: ",n,rdcount,sno);
		for(int i=0;i<nex;i++) { // compute leftsup, rightsup, maxleftsupport and maxrightsupport mainly --> loop could start at i=1 and avoid the next if
